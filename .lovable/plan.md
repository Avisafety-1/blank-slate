...
Feltoversikten du lastet opp bekrefter navnene: `SERIAL.battery` ("serial number of the aircraft battery") og `SERIAL.battery2` ("serial number of the second aircraft battery"). `BATTERY1.*`/`BATTERY2.*` har ingen egne serienummer-felt, så SERIAL-feltene er eneste kilde til SN for pakke 2. Merk at den offisielle listen kun har `DETAILS.batterySerial` (ikke `DETAILS.batterySN`) — begge beholdes i koden som i dag.
...
- Serienummer for batteri 1 velges slik: bruk `SERIAL.battery` hvis den finnes **og** er minst like komplett som `DETAILS.batterySerial`/`DETAILS.batterySN`; ellers brukes dagens verdi. Samme "aldri erstatt et lengre SN med et kortere"-prinsipp som allerede gjelder for droner i `droneLogMatching.ts`, så et fullt 20-tegns SN vinner over det avkortede 16-tegns.
- Finnes ikke de nye feltene i responsen, oppfører alt seg nøyaktig som i dag.

### 2. Batteri 2 gjennom hele kjeden

- Parseren returnerer i tillegg `battery2SN` (fra `SERIAL.battery2`) ved siden av de allerede parsede verdiene fra `BATTERY2.timesCharged`, `BATTERY2.fullCapacity [mAh]`, `BATTERY2.maxCellVoltageDeviation [V]`, `BATTERY2.voltage [V]` og `BATTERY2.maxTemperature [C]`.
