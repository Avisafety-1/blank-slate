...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // ─── Action: raw-test ───
    if (action === "raw-test") {
      const { testUrl, testHeaderName, testToken } = params;
      const effectiveUrl = testUrl || `${fh2BaseUrl}/openapi/v0.1/project?page=1&page_size=100`;
      const effectiveHeaderName = testHeaderName || "X-User-Token";
      const effectiveToken = testToken || fh2Token;

      console.log(`=== RAW TEST ===`);
      console.log(`URL: ${effectiveUrl}`);
      console.log(`Header: ${effectiveHeaderName}`);
      console.log(`Token length: ${effectiveToken.length}`);
      console.log(`Token fingerprint: ${effectiveToken.substring(0, 10)}...${effectiveToken.substring(effectiveToken.length - 6)}`);

      const reqHeaders: Record<string, string> = {
        [effectiveHeaderName]: effectiveToken,
        "X-Request-Id": crypto.randomUUID(),
        "X-Language": "en",
      };

      console.log(`Request headers:`, JSON.stringify(Object.keys(reqHeaders)));

      try {
        const res = await safeFetch(effectiveUrl, { method: "GET", headers: reqHeaders });
        const bodyText = await res.text();
        ...
        return new Response(JSON.stringify({
          request: {
            url: effectiveUrl,
            method: "GET",
            headers: Object.keys(reqHeaders),
            auth_header: effectiveHeaderName,
            token_length: effectiveToken.length,
            token_fingerprint: `${effectiveToken.substring(0, 10)}...${effectiveToken.substring(effectiveToken.length - 6)}`,
          },
          response: {
            status: res.status,
            headers: respHeaders,
            body: parsedBody || bodyText.substring(0, 1000),
          },
        }), {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
        ...
        return new Response(JSON.stringify({
          request: { url: effectiveUrl, auth_header: effectiveHeaderName },
          error: err.message,
        }), {
          status: 502, headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
