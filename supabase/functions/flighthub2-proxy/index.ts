  ...
  try {
    ...
    if (action === "list-devices") {
      ...
      for (const v of API_VARIANTS) {
        ...
        // ─── Step C: probe pilot/aircraft endpoints (covers manually flown drones not bound to a dock) ───
        // DJI FlightHub 2 OpenAPI does not expose pilot-flown aircraft via /device. We try a set of
        // candidate endpoints; any 404/unsupported is silently ignored. Successes get merged in.
        try {
          const projectAttempt = await tryListProjects(fh2BaseUrl, fh2Token, v, safeFetch);
          const projects = extractListPayload(projectAttempt.data?.data);
          const projectsToTry = projects.length > 0 ? projects : [{ uuid: jwtProjectUuid || projectUuid }];

          // Per-project candidates (use {uuid} placeholder)
          const perProjectCandidates = v.name === "openapi-v0.1"
            ? [
                "/openapi/v0.1/aircraft?page=1&page_size=200",
                "/openapi/v0.1/project/aircraft?page=1&page_size=200",
                "/openapi/v0.1/pilot/device?page=1&page_size=200",
                "/openapi/v0.1/workspaces/{uuid}/devices?page=1&page_size=200",
                "/openapi/v0.1/devices/topo",
              ]
            : [
                "/manage/api/v1.0/aircraft?page=1&page_size=200",
                "/manage/api/v1.0/pilot/device?page=1&page_size=200",
                "/manage/api/v1.0/workspaces/{uuid}/devices?page=1&page_size=200",
              ];
          ...
          for (const project of projectsToTry) {
            ...
            for (const candidate of perProjectCandidates) {
              const path = candidate.replace("{uuid}", currentProjectUuid);
              const url = `${fh2BaseUrl}${path}`;
              try {
                const headers = makeHeaders(v, true, currentProjectUuid);
                const res = await safeFetch(url, { method: "GET", headers });
                const text = await res.text();
                let data: any = null;
                try { data = JSON.parse(text); } catch { /* ignore */ }

                const rawList = extractListPayload(data?.data);
                const flatList = flattenDjiDeviceList(rawList);
                const normalized = flatList.map((d: any) => normalizeDevicePayload(d, project));

                diagnostics.push({
                  variant: v.name,
                  stage: "pilot-aircraft-probe",
                  url,
                  project_uuid: currentProjectUuid,
                  status: res.status,
                  code: data?.code ?? null,
                  ...
                  raw_body_preview: text.substring(0, 300),
                });

                if (res.ok && data?.code === 0 && normalized.length > 0) {
                  workingVariant = workingVariant || v.name;
                  for (const device of normalized) {
                    const key = device.device_sn || `${device.device_name}-${currentProjectUuid}`;
                    if (key && !devicesByKey.has(key)) {
                      devicesByKey.set(key, { ...device, _source_endpoint: path });
                    }
                  }
                }
              } catch (err: any) {
          ...
          diagnostics.push({ variant: v.name, stage: "pilot-aircraft-probe", error: err.message });
        }
