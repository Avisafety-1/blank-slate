  ...
  FOR v_feature IN SELECT * FROM jsonb_array_elements(p_features) LOOP
    BEGIN
      INSERT INTO public.dk_drone_zones (
        ...
        ST_SetSRID(ST_GeomFromGeoJSON(v_feature->>'geometry_geojson'), 4326),
...
-- Trigger initial sync now that the functions are fixed
DO $$
...
BEGIN
  ...
  PERFORM net.http_post(
    ...
    body := jsonb_build_object('trigger', 'manual-initial-v2', 'time', now())
  );
END $$;