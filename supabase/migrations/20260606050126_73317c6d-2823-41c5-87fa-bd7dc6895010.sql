...
BEGIN
  ...
  -- Keep drone_documents link rows aligned with the drone's new department
  -- so RLS doesn't hide them and re-linking doesn't hit UNIQUE(drone_id,document_id).
...
-- Backfill: rett opp drone_documents-rader som ble igjen i kildeavdelingen
-- ved tidligere droneflyttinger (før denne fiksen).
