...
BEGIN
  ...
  -- Check AIP restriction zones - use name as display name instead of zone_id
  FOR v_aip_zone IN 
  ...
  LOOP
    ...
    v_aip_display_name := COALESCE(v_aip_zone.name, v_aip_zone.zone_id, 'Ukjent');
    ...
    IF v_aip_zone_inside THEN
      IF v_aip_zone.zone_type = 'P' THEN
        v_results := v_results || jsonb_build_object(
          ...
          'zone_name', v_aip_display_name,
          'distance_meters', 0, 'is_inside', true, 'level', 'warning',
          'message', 'Oppdraget er innenfor forbudsområde ' || v_aip_display_name || '. Flyving ikke tillatt uten dispensasjon fra Luftfartstilsynet.'
        );
      ELSIF v_aip_zone.zone_type = 'R' THEN
        v_results := v_results || jsonb_build_object(
          ...
          'zone_name', v_aip_display_name,
          'distance_meters', 0, 'is_inside', true, 'level', 'warning',
          'message', 'Oppdraget er innenfor restriksjonsområde ' || v_aip_display_name || '. Sjekk vilkår og innhent tillatelse.'
        );
      ELSE
        v_results := v_results || jsonb_build_object(
          ...
          'message', 'Oppdraget er innenfor fareområde ' || v_aip_display_name || '. Vurder aktivitetsstatus via NOTAM.'
        );
      END IF;
    ELSIF v_aip_zone_worst_distance IS NOT NULL AND v_aip_zone_worst_distance <= 5000 THEN
      v_results := v_results || jsonb_build_object(
        ...
        'message', 'Nærmeste AIP ' || CASE v_aip_zone.zone_type WHEN 'P' THEN 'forbudsområde' WHEN 'R' THEN 'restriksjonsområde' ELSE 'fareområde' END || ' (' || v_aip_display_name || ') er ' || ROUND(v_aip_zone_worst_distance::numeric) || ' m unna.'
      );
    END IF;
  END LOOP;
