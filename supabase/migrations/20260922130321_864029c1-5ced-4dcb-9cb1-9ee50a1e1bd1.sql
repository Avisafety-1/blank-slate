  ...
  FOR _action IN SELECT * FROM jsonb_array_elements(_actions) LOOP
    ...
    IF _act = 'move' THEN
      ...
      ELSIF _type = 'document' THEN
        ...
        IF _doc_owner = _from_company_id THEN
          ...
          -- but keep the source department's read access so remaining drones
          -- and missions that still reference it do not lose the document.
          ...
          VALUES (_resource_id, _from_company_id)
    ...
    ELSIF _act = 'leave' THEN
      ...
      ELSIF _type = 'document' THEN
        ...
        -- Also detach the document when it is referenced as a checklist on the drone
        -- itself, otherwise the drone arrives with a checklist the receiving
        -- department cannot see, open or remove.
        UPDATE public.drones
           SET operations_checklist_ids = array_remove(COALESCE(operations_checklist_ids, ARRAY[]::text[]), _resource_id::text),
               post_flight_checklist_id = CASE WHEN post_flight_checklist_id = _resource_id THEN NULL ELSE post_flight_checklist_id END,
               sjekkliste_id = CASE WHEN sjekkliste_id = _resource_id THEN NULL ELSE sjekkliste_id END,
               operations_checklist_id = CASE WHEN operations_checklist_id = _resource_id THEN NULL ELSE operations_checklist_id END
         WHERE id = _drone_id;
      END IF;
