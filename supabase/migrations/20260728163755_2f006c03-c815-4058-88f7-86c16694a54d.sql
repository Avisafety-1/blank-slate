...
BEGIN
  ...
  ELSE
    ...
    RETURN QUERY
      SELECT DISTINCT p.id, p.full_name, p.email, p.company_id, c.navn AS company_name
      FROM public.profiles p
      LEFT JOIN public.companies c ON c.id = p.company_id
      WHERE p.id <> auth.uid()
        AND (
          p.company_id = ANY (visible_ids)
          OR EXISTS (
            SELECT 1 FROM public.user_roles ur
            WHERE ur.user_id = p.id AND ur.role = 'superadmin'::app_role
          )
        )
        AND (q = '' OR p.full_name ILIKE pattern OR p.email ILIKE pattern)
      ORDER BY p.full_name NULLS LAST
