...
AS $$
  ...
  admins AS (
    ...
    WHERE ur.role IN ('admin', 'administrator')
  ),
  home AS (
    -- Primary source of truth: user_companies (stable home membership)
    SELECT a.uid, uc.company_id
    FROM admins a
    JOIN public.user_companies uc ON uc.user_id = a.uid
    UNION ALL
    -- Fallback for legacy accounts with no user_companies rows
...
GRANT EXECUTE ON FUNCTION public.get_pending_approval_notification_admins(uuid) TO service_role, authenticated;