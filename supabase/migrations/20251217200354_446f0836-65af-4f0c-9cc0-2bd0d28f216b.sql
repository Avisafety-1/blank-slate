-- Update function to get airport name from nearest rpas_5km_zone for CTR/TIZ
...
BEGIN
  ...
  -- If CTR zone name is null/unknown, look up nearest airport from rpas_5km_zones
  IF v_ctr_worst_point IS NOT NULL AND (v_ctr_zone_name IS NULL OR v_ctr_zone_name = 'Ukjent' OR v_ctr_zone_name = '') THEN
    SELECT name INTO v_nearest_airport
    FROM rpas_5km_zones
    WHERE name IS NOT NULL AND name != ''
    ORDER BY ST_Distance(geometry::geography, v_ctr_worst_point::geography)
    LIMIT 1;
    
    IF v_nearest_airport IS NOT NULL THEN
      v_ctr_zone_name := v_nearest_airport;
    END IF;
  END IF;
  ...
  IF v_ctr_worst_inside THEN
    v_results := v_results || jsonb_build_object(
      'zone_type', COALESCE(v_ctr_zone_type, 'CTR/TIZ'),
      'zone_name', COALESCE(v_ctr_zone_name, 'ukjent'),
      'distance_meters', 0,
      'is_inside', true,
      'level', 'warning',
