...
BEGIN
  ...
  -- If the switch is within the same hierarchy (parent↔child), do nothing
  IF EXISTS (
    SELECT 1 FROM companies 
    WHERE id = NEW.company_id AND parent_company_id = OLD.company_id
  ) OR EXISTS (
    SELECT 1 FROM companies 
    WHERE id = OLD.company_id AND parent_company_id = NEW.company_id
  ) THEN
    RETURN NEW;
  END IF;

  -- Permanent reassignment: clean up old access
