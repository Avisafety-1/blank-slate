CREATE OR REPLACE FUNCTION public.get_platform_statistics(p_exclude_company_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_risk_assessments_per_month jsonb := '[]'::jsonb;
...
BEGIN
  ...
  -- Count ALL risk assessments (including multiple per mission)
  SELECT count(*) INTO v_total_risk_assessments
  ...
  FOR i IN REVERSE 11..0 LOOP
    ...
    v_risk_assessments_per_month := v_risk_assessments_per_month || jsonb_build_object(
      'month', v_month_label,
      'count', (SELECT count(*) FROM mission_risk_assessments
                WHERE created_at >= v_month_start AND created_at < v_month_start + interval '1 month'
                AND (p_exclude_company_id IS NULL OR company_id != p_exclude_company_id))
    );
  END LOOP;
  ...
  result := jsonb_build_object(
    'kpis', jsonb_build_object(
      ...
      'totalRiskAssessments', v_total_risk_assessments
    ),
    'trends', jsonb_build_object(
      ...
      'riskAssessmentsPerMonth', v_risk_assessments_per_month
    ),
