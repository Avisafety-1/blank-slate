CREATE OR REPLACE FUNCTION public.check_mission_airspace(p_lat double precision, p_lng double precision, p_route jsonb DEFAULT NULL)
...
BEGIN
  ...
  WITH candidate_zones AS (
    ...
    -- Active NOTAMs: use NOTAM ID + clean text summary (strip HTML and Q-line)
    SELECT
      nt.notam_id::text,
      'NOTAM',
      nt.notam_id || ': ' || LEFT(
