...
BEGIN
  ...
  WITH candidate_zones AS (
    ...
    UNION ALL
    -- ATZ småflyplasser fra OpenAIP: 5 km sirkel rundt sentroide av ATZ-polygonen.
    ...
    UNION ALL
    -- Småflyplasser fra Luftfartstilsynets liste (dronesoner.no, layer "flyplasser"):
    -- behandles som ATZ_5KM med 5 km sirkel rundt sentroide.
    -- Avinor-flyplasser er allerede dekket av rpas_5km_zones og blir ikke duplisert
    -- siden de fortsatt får '5KM'-warning med høyere alvorlighet.
    ...
    UNION ALL
    -- CAA-soner (fengsler, ambassader, fareområder, NOTAM-soner) — flyplasser håndteres separat over.
    SELECT cz.id::text,
  ...
  SELECT
    ...
    CASE
      ...
      WHEN rc.cz_type IN ('CAA_FAREOMRADER', 'CAA_NOTAM_SONER') THEN 'CAUTION'
      ELSE 'INFO'
    END
  FROM route_check rc
