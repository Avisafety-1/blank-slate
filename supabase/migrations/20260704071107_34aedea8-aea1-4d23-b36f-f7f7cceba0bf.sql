-- Add default map layers company setting with optional propagation
ALTER TABLE public.companies
  ADD COLUMN IF NOT EXISTS default_map_layers jsonb NOT NULL DEFAULT '{}'::jsonb,
  ADD COLUMN IF NOT EXISTS propagate_default_map_layers boolean NOT NULL DEFAULT false;

-- Recreate propagation trigger byte-for-byte from current DB definition,
-- appending only a new default_map_layers block before RETURN NEW.
...
BEGIN
  ...
  IF COALESCE(NEW.propagate_default_map_layers, false) AND (
       ...
       OR COALESCE(NEW.propagate_default_map_layers, false) IS DISTINCT FROM COALESCE(OLD.propagate_default_map_layers, false)
  ) THEN
    UPDATE public.companies
