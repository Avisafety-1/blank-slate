...
// Supabase native passkey API is experimental and not yet in the published TS types.
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (remaining.length === 0) {
        try {
          localStorage.removeItem("avisafe_passkey_registered");
        } catch {}
      }
          ...
          {isDevEnv && (
            <Alert className="border-amber-500/50 bg-amber-500/10">
              ...
              <AlertDescription className="text-xs sm:text-sm">
                {t(
                  "passkey.devWarning",
                  "Passkeys må registreres via app.avisafe.no for å fungere korrekt. De kan ikke brukes fra forhåndsvisning eller utviklingsmiljø."
                )}
              </AlertDescription>
            </Alert>
          )}
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs sm:text-sm font-medium truncate">
                        {pk.friendly_name || t("passkey.unknownDevice")}
                      </p>
