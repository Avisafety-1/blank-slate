    ...
    <Dialog open={profileDialogOpen} onOpenChange={setProfileDialogOpen}>
      <DialogTrigger asChild>
        <Button variant="ghost" size="sm" title={t('profile.title')} className="relative h-7 w-7 min-w-7 p-0 md:h-8 md:w-8">
          ...
          {(followUpIncidents.length + pendingApprovalMissions.length + pendingTraining.length + unreadInbox) > 0 && (
            <Badge
              variant="destructive"
              className="absolute -top-1 -right-1 h-5 w-5 flex items-center justify-center p-0 text-xs rounded-full"
            >
              {followUpIncidents.length + pendingApprovalMissions.length + pendingTraining.length + unreadInbox}
            </Badge>
          )}
        </Button>
      </DialogTrigger>
        ...
        <ScrollArea className="max-h-[calc(90vh-100px)] w-full min-w-0 pr-0 sm:pr-4 [&>[data-radix-scroll-area-viewport]]:!block [&>[data-radix-scroll-area-viewport]]:w-full [&>[data-radix-scroll-area-viewport]]:min-w-0">
          {loading ? (
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <TabsList className={`grid w-full grid-cols-3 md:grid-cols-4 gap-1.5 p-1.5 lg:p-1 bg-transparent lg:bg-muted relative z-10 h-auto lg:h-10 ${canBeIncidentResponsible ? (stripeExempt ? 'lg:grid-cols-7' : 'lg:grid-cols-8') : (stripeExempt ? 'lg:grid-cols-6' : 'lg:grid-cols-7')}`}>
              ...
              <TabsContent value="notifications" className="space-y-4 mt-4 min-h-[400px] sm:min-h-0">
                ...
                {/* ── Oppdragsvarsler (flytur + oppdragsstart) ───────────── */}
                <Card className="overflow-hidden border-primary/25">
                  <CardHeader className="bg-primary/5 border-b border-primary/15 p-4">
                    <CardTitle className="flex items-center gap-2 text-base">
                      <Plane className="h-4 w-4 text-primary shrink-0" />
                      {t('profile.notificationSections.operations')}
                    </CardTitle>
                    <p className="text-xs text-muted-foreground">
                      {t('profile.notificationSections.operationsDesc')}
                    </p>
                  </CardHeader>
                  <CardContent className="p-4 space-y-4">
                    ...
                    <div className="rounded-lg border border-border bg-muted/20 p-3 sm:p-4 space-y-3">
                      <div className="flex items-start gap-2">
                        ...
                        <div className="space-y-0.5">
                          <h4 className="text-sm font-semibold leading-tight">{t('profile.longFlightAlerts')}</h4>
                    ...
                    <div className="rounded-lg border border-border bg-muted/20 p-3 sm:p-4 space-y-3">
                      <div className="flex items-start gap-2">
                        ...
                        <div className="space-y-0.5">
                          <h4 className="text-sm font-semibold leading-tight">{t('profile.missionStartAlerts')}</h4>
                      ...
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <Input
                          ...
                          className="w-24 h-9 text-center shrink-0"
                        />
                      </div>
                    </div>
                  </CardContent>
                ...
                <Card className="overflow-hidden">
                  <CardHeader className="bg-muted/40 border-b border-border p-4">
                    <CardTitle className="flex items-center gap-2 text-base">
                      ...
                      {t('profile.notificationSections.email')}
                    </CardTitle>
                    <p className="text-xs text-muted-foreground">
                      {t('profile.notificationSections.emailDesc')}
                    </p>
                  </CardHeader>
                  ...
                  <Card className="overflow-hidden">
                    <CardHeader className="bg-muted/40 border-b border-border p-4">
                      <CardTitle className="flex items-center gap-2 text-base">
                        ...
                        {t('profile.notificationOptions.childCompanies')}
                      </CardTitle>
                      <p className="text-xs text-muted-foreground">
                ...
                <Card className="overflow-hidden">
                  <CardHeader className="bg-muted/40 border-b border-border p-4">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t('profile.notificationSections.pushDesc')}
                    </p>
                  </CardHeader>
                  <CardContent className="p-4">
                    {!pushSupported ? (
