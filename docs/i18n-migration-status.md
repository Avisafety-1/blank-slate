...
| 5.3 | Edge functions (bruker-vendte) → lokale `prompts.ts` + tospråklige e-postmaler | DONE | AI: company-status-ai og platform-statistics-ai fikk `prompts.ts` 2026-07-13. E-post: 2026-07-13 utvidet alle 11 e-post-funksjoner (`send-user-welcome-email`, `send-customer-welcome-email`, `send-user-approved-email`, `send-password-reset`, `invite-user`, `resend-confirmation-email`, `send-notification-email`, `send-feedback`, `preview-currency-emails`, `send-template-previews`, `test-email`) med `language`-støtte via ny `_shared/email-i18n.ts` + `_shared/default-templates-en.ts` (engelsk tvilling av alle 14 default-maler). `email_templates`-tabellen har fått `language`-kolonne (composite unique på (company_id, template_type, language)). `EmailTemplateEditor.tsx` har språkvelger. Klient-siden bruker `src/lib/emailInvoke.ts` wrapper (`invokeEmailFunction`) som auto-legger på `language: i18n.language` – 11 kall-steder migrert. `check-currency-status` (cron) har egne inline templates og må porteres neste gang funksjonen røres. |
