...
export const Shot: React.FC<{
  ...
  const enterX = clipFrom === "left" ? -48 : clipFrom === "right" ? 48 : 0;
  const enterY = clipFrom === "bottom" ? 48 : 0;
  ...
  return (
    <div
      style={{
        ...
        opacity: p,
        transform: `rotate(${rotate}deg) translate3d(${(1 - p) * enterX}px, ${(1 - p) * enterY}px, 0)`,
        background: C.navy,
        ...style,
      }}
    >
      <Img
        src={staticFile(src)}
        style={{ width: "100%", display: "block", transform: `scale(${scale})`, backfaceVisibility: "hidden" }}
      />
    </div>
  );
};
